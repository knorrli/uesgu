...
> KB via `script/subset_phosphor.py`), and **iOS splash screens** — 18
> `apple-touch-startup-image` PNGs (current iPhones + iPads, portrait; the üsgu
> mark on `#17131a`) generated by `script/generate_ios_splash.rb`, wired via the
> generated partial `layouts/_ios_splash_screens`. D (CSS minify) **declined** —
> needs a build step we don't want.
>
> **CSS-class trim now done too (folded into the font subset):**
> `script/subset_phosphor.py` regenerates BOTH artifacts from the same used-set —
> the font subset *and* a trimmed `phosphor.css` (~78 KB → ~2 KB, off the
> render-blocking bundle). The full masters live in `vendor/phosphor/`
> (`Phosphor.woff2` + `phosphor.css`, never served); the served files in
> `app/assets/` are build outputs. Same regen cadence as the font (re-run only
> when adding/removing an icon), and the two artifacts can't drift.
>
> Gotcha for the splash generator: rasterise the mark with `rsvg-convert`, **not**
> ImageMagick — IM's SVG path renders the light-cone gradients as solid black and
> ignores the resize. The script composites the rsvg output onto the canvas.
