...
venues.each do |v|
  ...
  begin
    Timeout.timeout(PER_VENUE_TIMEOUT) do
      ...
      genre_p_adds   = matched.count { |m| m[:scraper][:genres].empty? && m[:petzi][:genres].any? }
      genre_s_adds   = matched.count { |m| m[:petzi][:genres].empty? && m[:scraper][:genres].any? }
